...
public class ActivationCodeServiceImpl implements ActivationCodeService, CommonUpdateService<ActivationCode> {
     ...
     */
    @Override
    public Flux<ActivationCode> findById(UUID id) {
        return Flux.from(activationCodeRepository.findById(id))
                .switchIfEmpty(Mono.error(
                                new APIException(
                                        "Код активации с id = %s не найден".formatted(id.toString()),
     ...
     */
    @Override
    public Flux<ActivationCode> findAll() {
        return Flux.from(activationCodeRepository.findAll());
    }
     ...
     */
    @Override
    public Flux<ActivationCode> save(ActivationCode entity) {
        return Flux.from(activationCodeRepository.save(entity));
    }
    ...
    public ActivationCode findByActivationCode(String activationCode) {
        return Flux.from(activationCodeRepository.findByActivationCode(activationCode))
                .switchIfEmpty(Mono.error(
                                new APIException(
     ...
     */
    @Override
    public Flux<ActivationCode> update(ActivationCode entity) {
        return Flux.from(activationCodeRepository.update(entity));
    }
}