...
public class RefreshTokenServiceImpl implements RefreshTokenService {
     ...
     */
    @Override
    public Flux<RefreshToken> findById(UUID id) {
        return Flux.from(refreshTokenRepository.findById(id))
                .switchIfEmpty(Mono.error(
                                new APIException(
                                        "Код обновления с id = %s не найден".formatted(id),
    ...
    public Flux<RefreshToken> findAll() {
        return Flux.from(refreshTokenRepository.findAll());
    }
     ...
     */
    @Override
    public Flux<RefreshToken> save(RefreshToken entity) {
        return Flux.from(refreshTokenRepository.save(entity));
    }
    ...
    public String generateRefreshToken(Authentication authentication, String refreshTokenKey) throws APIException {
        return refreshTokenGenerator.generate(authentication, refreshTokenKey).orElseThrow(() -> new APIException(
                "Не удалось сгенерировать токен обновления!",
