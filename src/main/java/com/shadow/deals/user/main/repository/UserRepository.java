...
public interface UserRepository extends CommonRepository<User> {
     ...
     */
    @Query(value = """
            SELECT * FROM kast_user WHERE id = (SELECT user_id FROM kast_refresh_token WHERE refresh_token = :refreshTokenKey)
            """,
            nativeQuery = true)
    Publisher<User> findByRefreshTokenKey(String refreshTokenKey);
