...
public class AuthServiceImpl implements AuthService {
    ...
    /**
     * An instance of the interface that provides the ability to interact with the {@link RefreshToken}
     * entity.
     */
    private final RefreshTokenService refreshTokenService;

    /**
     * An instance of the interface that provides the ability to interact with the {@link UserRole}
     * entity.
     */
     ...
     */
    @Override
    public Flux<String> signup(@NotNull SignUpRequestDTO signUpRequestDTO) throws APIException {
        String userEmail = signUpRequestDTO.getEmail();
        if (userService.existsByEmail(userEmail)) {
            throw new APIException(
                    "Пользователь с почтой %s уже существует".formatted(userEmail)
        ...
        Flux<User> user = mapSignUpRequestDTOToUser(signUpRequestDTO);
    ...
    public Publisher<MutableHttpResponse<?>> signin(@NotNull SignInRequestDTO signInRequestDTO, HttpRequest<?> request) {
        String authUnexpectedError = "Во время авторизации произошла непредвиденная ошибка!";
    ...
    public String changeEmail(@NotNull UpdateEmailRequestDTO updateEmailRequestDTO) throws APIException {
        ...
        if (activationCode == null) {
            ...
            createAndSaveActivationCode(Flux.just(user), activationCodeVal);
        } else {
    ...
    public void changePassword(@NotNull ChangePasswordRequestDTO changePasswordRequestDTO) throws APIException {
        ...
        if (!generateUUIDFromString(email).equals(changePasswordRequestDTO.getChangePasswordCode().toString())) {
            throw new APIException(
                    "Код для смены пароля не принадлежит почте %s".formatted(email),
                    HttpStatus.BAD_REQUEST);
        }
    ...
    public void logout(@NotNull HttpRequest<?> request) throws APIException {
        ...
        try {
            ...
            throw new APIException(
                    "Во время обработки JWT токена произошла ошибка",
     ...
     * @param activationCodeVal the activation code for email confirmation.
     */
    private void createAndSaveActivationCode(Flux<User> user, String activationCodeVal) {
        ActivationCode activationCode = new ActivationCode();
        activationCode.setActivationCode(activationCodeVal);
        activationCode.setUser(user.blockFirst());
     ...
     *                      <li>• The role {@link UserRoleName#USER} does not exist in the database.</li>
     */
    private Flux<User> mapSignUpRequestDTOToUser(SignUpRequestDTO signUpRequestDTO) throws APIException {
    ...
    private String generateAccessToken(Authentication authentication, int expiration) throws APIException {
        ...
        if (optionalAccessToken.isEmpty()) {
            throw new APIException("Произошла непредвиденная ошибка во время генерации токена доступа", HttpStatus.INTERNAL_SERVER_ERROR);
        }
